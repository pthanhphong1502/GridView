...
export class SelectScrollBarComponent extends UIComponent {
  ...
  navChange(evt: any, index: number = -1, functionID: number = -1, btnClick: any) {
    console.log(evt)
    let abc = document.getElementById('pane_2');
    let pan_3 = document.getElementById('pane_3');

    let parentContainer = document.querySelectorAll('.tab-pane.fade.show');
    ...
    if (distanceToBottom < containerHeight) {
      ...
      (lastDiv as any).style.height = `${containerHeight - distanceToBottom + 150
        }px`;
    }
    ...
    //   inline: 'nearest'
    // });
    parentContainer[0].scrollIntoView({
      behavior: 'smooth',
      block: 'nearest',
      inline: 'nearest'
    });
